...
<body>
  <div class="page">
    ...
    <div class="footer">
      Generated by <strong>Ace Digital</strong> · © ${new Date().getFullYear()} · acedigital.cc
    </div>
  </div>
</body>
</html>`;
...
function buildRevenueReport(
  ...
  const avgDealSize =
    activeClients.length > 0
      ...
      : 0;
  const totalBudget = showProjectBudget
  ...
  const body = `
    <div class="kpi-grid">
      ...
      ${showProjectBudget ? `<div class="kpi"><div class="label">Project Budget Total</div><div class="value amber">${fmtCurrency(totalBudget)}</div></div>` : ""}
    </div>
...
function buildExpenseReport(
  ...
  const totalExpenses = showProjectBudget
    ? totalBudget + totalSalaryExpense
    : totalSalaryExpense;

  const body = `
    <div class="kpi-grid">
      ${showProjectBudget ? `<div class="kpi"><div class="label">Project Budgets</div><div class="value blue">${fmtCurrency(totalBudget)}</div></div>` : ""}
      <div class="kpi"><div class="label">Salary Expenses</div><div class="value amber">${fmtCurrency(totalSalaryExpense)}</div></div>
      <div class="kpi"><div class="label">Total Expenses</div><div class="value red">${fmtCurrency(totalExpenses)}</div></div>
...
export default function ReportsPage() {
  ...
  const canViewProjectBudget = can("projects:budget");
  ...
  function handleDownload(report: Report) {
    ...
    try {
      switch (report.type) {
        case "REVENUE":
          html = buildRevenueReport(report, clients ?? [], projects ?? [], canViewProjectBudget);
          break;
        case "PAYROLL":
